...
import { allRevisions, corpus } from "@/lib/data";
...
export const metadata: Metadata = pageMetadata({
  title: "Atualizações",
  description: "Histórico de atualizações editoriais do corpus: registros adicionados, relações atualizadas e correções.",
  path: "/atualizacoes",
});
...
export default function AtualizacoesPage() {
  ...
  return (
    <PageShell>
      <PageTitle eyebrow="Transparência" title="Atualizações" lede={`Último build do corpus: ${formatDateTimeBRT(corpus.built_at)}. Cada alteração de dado passa por validação automática e revisão; o histórico completo está no Git do repositório.`} />
      {revisions.length === 0 ? (
          ...
          {revisions.map((r) => {
            ...
            return (
              <li key={r.id} className="relative pb-8 last:pb-0">
                <span aria-hidden="true" className="bg-accent absolute top-1.5 -left-[1.85rem] h-2.5 w-2.5 rounded-full" />
                ...
                <ul className="text-fg-3 mt-2 flex flex-wrap gap-x-4 gap-y-1 font-mono text-xs">
                  ...
                  {r.updated_relationships > 0 && <li>{r.updated_relationships} relações atualizadas</li>}
      ...
      <p className="text-fg-3 mt-10 text-xs">
        ...
        <a href={`${SITE.repository}/commits/main/data`} rel="noopener noreferrer" className="underline underline-offset-2">
