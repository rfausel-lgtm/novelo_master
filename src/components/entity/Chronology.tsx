...
const KINDS = ["event", "public_act", "transaction"] as const;
const MONTHS = [
  "janeiro",
  "fevereiro",
  "março",
...
/*
 * Os filtros vivem na URL (`agente`, `tipo`, `classe`), não em estado local: o link reproduz o
 * recorte, e voltar de um registro não o perde. Valor que não existe no corpus cai no padrão.
 */
export function Chronology({ items }: { items: TimelineItem[] }) {
  ...
  const agenteUrl = params.get("agente");
  const agent = agenteUrl && agents.some(([id]) => id === agenteUrl) ? agenteUrl : "all";
  const tipoUrl = params.get("tipo");
  const kind = tipoUrl && (KINDS as readonly string[]).includes(tipoUrl) ? tipoUrl : "all";
  const classes = listaDaUrl(params.get("classe"), CLASSES);
  const recorte = agent !== "all" || kind !== "all" || classes.size !== CLASSES.length;

  /* Algumas centenas de registros: filtrar e agrupar a cada render custa menos que memoizar. */
  const filtered = items.filter(
    (it) =>
      (agent === "all" || it.agents.some((a) => a.id === agent)) &&
      classes.has(it.evidenceClass) &&
  ...
  const toggle = (c: EvidenceClass) => {
    ...
    gravar({ classe: listaParaUrl(next, CLASSES) });
  };
    ...
    <div>
      <div className="border-border mb-6 flex flex-wrap items-end gap-4 border-y py-3 text-sm">
        <label className="flex flex-col gap-1 text-xs">
          ...
          <select
            value={agent}
            onChange={(e) => gravar({ agente: e.target.value === "all" ? null : e.target.value })}
            className="border-border-strong bg-bg-2 text-fg h-9 max-w-xs rounded-md border px-2"
          >
        ...
        <label className="flex flex-col gap-1 text-xs">
          ...
          <select
            value={kind}
            onChange={(e) => gravar({ tipo: e.target.value === "all" ? null : e.target.value })}
            className="border-border-strong bg-bg-2 text-fg h-9 rounded-md border px-2"
          >
            <option value="all">Todos</option>
        ...
        <fieldset className="flex items-center gap-3 text-xs">
          ...
          {CLASSES.map((c) => (
            <label key={c} className="text-fg-2 flex items-center gap-1">
              ...
              <span className="sr-only">{EVIDENCE_CLASS_LABEL[c]}</span>
            </label>
          ))}
        </fieldset>
        ...
        <div className="space-y-8">
          {groups.map(([label, list]) => (
            <section key={label} aria-labelledby={`g-${label}`}>
              <h2
                id={`g-${label}`}
                className="text-fg-3 sticky top-14 z-10 mb-2 bg-[var(--bg)] py-1 font-mono text-[11px] tracking-[0.2em] uppercase"
              >
                {label}
              </h2>
              {/*
