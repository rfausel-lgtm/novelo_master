...
import { CopiarDossie } from "./CopiarDossie";
import { tamanhoDossie } from "@/lib/data/dossies";
...
const FAMILY_ORDER: RelationshipFamily[] = [
  "institutional",
  "corporate",
  "financial",
  "political",
  ...
  return (
    <article>
          ...
          */}
          <details className="border-border mb-6 rounded border px-3 py-2">
            ...
            <p className="text-fg-3 mt-2 text-xs">
              Detalhe em{" "}
              <Link href="/metodologia" className="hover:text-fg underline underline-offset-2">
                metodologia
              </Link>
              .
            </p>
          ...
          <CopiarDossie href={`/dossies/${id}.txt`} bytes={tamanhoDossie(id)} />

          <Section id="resumo" title="Resumo">
            <p className="prose-novelo text-fg-2 text-sm leading-relaxed sm:text-base">
              {entity.summary}
            </p>
                  ...
                  <li key={i} className="text-fg-2">
                    ...
                    {(p.organization_id || p.organization) && (
                      <>
                        ...
                        {p.organization_id && getOrganization(p.organization_id) ? (
                          <Link
                            href={`/organizacoes/${p.organization_id}`}
                    ...
                    {(p.start_date || p.end_date) && (
                      <span className="text-fg-3 text-xs">
                        {" "}
                        ({p.start_date ? formatPartialDate(p.start_date) : "?"} –{" "}
                        {p.end_date ? formatPartialDate(p.end_date) : "atual"})
                      </span>
                    )}
                  </li>
                ))}
          ...
          <Section id="por-que" title="Por que está no Novelo?">
            <p className="border-accent/60 bg-bg-2/60 text-fg max-w-[68ch] rounded border-l-2 px-4 py-3 text-sm leading-relaxed sm:text-base">
              {entity.why_in_novelo}
            </p>
          </Section>

          <Section
            id="posicao"
            title="Posição do citado"
            description="Negativas, esclarecimentos, notas públicas ou versões apresentadas pela pessoa ou organização. Exibidas sempre, mesmo quando enfraquecem uma hipótese."
          >
          ...
          <Section
            id="lacunas"
            title="Lacunas ainda não esclarecidas"
            description="O que o corpus não demonstra. Se não sabemos, dizemos que não sabemos."
          >
      ...
      */}
          <Section
            id="conexoes"
            title="Mais bem documentadas"
            count={Math.min(rels.length, 12)}
            description="As doze relações com a evidência mais forte. Todas aparecem completas, agrupadas por categoria, na seção seguinte."
          >
            {rels.length === 0 ? (
                ...
                {rels.slice(0, 12).map((r) => {
                  ...
                  return (
                    <li key={r.id} className="flex flex-wrap items-center gap-x-2 gap-y-1 py-2">
                      ...
                      <Link
                        href={entityHref(outro)}
                        className="text-fg hover:text-accent font-medium underline-offset-2 hover:underline"
                      >
                        {entityName(outro)}
                      </Link>
                      ...
                      {r.start_date ? (
                        <span className="text-fg-3 font-mono text-[11px]">
                          {formatPartialDate(r.start_date)}
                        </span>
                      ) : null}
                    </li>
                  );
                  ...
                  <li key={t.id} className="py-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-fg-2 font-mono text-xs">
                        {formatPartialDate(t.date, t.date_precision)}
                      </span>
          ...
          <Section
            ...
            title="Evidências"
            count={evs.length}
            description="Cada evidência sustenta uma proposição concreta, com classificação própria."
          >
            {evs.length === 0 ? (
              ...
              <Dobra
                ...
                itens={evs.map((e) => (
                  <li key={e.id} className="border-border rounded border px-3 py-2">
                    ...
                    {e.excerpt && (
                      <blockquote className="text-fg-3 border-border mt-1 border-l pl-3 text-xs italic">
                        {e.excerpt}
                      </blockquote>
                    )}
                    {(e.attributed_to || e.attributed_to_id) && (
                      <p className="text-fg-3 mt-1 text-xs">
                        Atribuída a:{" "}
                        {e.attributed_to_id ? entityName(e.attributed_to_id) : e.attributed_to}
                      </p>
                    )}
                    {e.inference_basis && (
                      <p className="text-fg-3 mt-1 text-xs">
                        Base da inferência: {e.inference_basis}
                      </p>
                    )}
                    <p className="text-fg-3 mt-1 text-xs">
                      {e.document_ids.map((d) => (
                        <Link
                          key={d}
                          href={`/documentos/${d}`}
                          className="hover:text-fg mr-2 underline-offset-2 hover:underline"
                        >
                          documento
                        </Link>
                      ))}
                  ...
                  {claims.map((c) => (
                    <li key={c.id} className="border-border rounded border px-3 py-2">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {c.claimant && (
                          <span className="text-fg-3 text-xs">segundo {c.claimant}</span>
                        )}
                      </div>
                      <p className="text-fg-2 mt-1">{c.statement}</p>
                      {c.limits && (
                        <p className="text-fg-3 mt-1 text-xs">
                          O que os documentos não permitem afirmar: {c.limits}
                        </p>
                      )}
                      {c.adversarial_review && (
                        <p className="text-fg-3 mt-1 text-xs">
                          Revisão adversarial ({formatPartialDate(c.adversarial_review.reviewed_at)}
                          ): {c.adversarial_review.attempted_refutation} Resultado:{" "}
                          {c.adversarial_review.outcome}.
                        </p>
                      )}
                      <div className="mt-2">
          ...
          <Section
            ...
            description={`${official} fonte(s) primária(s) oficial(is).`}
          >
