...
export interface GraphState {
  ...
  multiSelect: boolean;
  selection: string[];
  focus: { root: string; depth: 1 | 2 } | null;
...
export type GraphAction =
  ...
  | { type: "clearSelection" }
  | { type: "focus"; root: string; depth: 1 | 2 }
  | { type: "clearFocus" }
...
export function graphReducer(state: GraphState, action: GraphAction): GraphState {
  switch (action.type) {
    ...
    case "toggleCategory":
      return { ...state, filters: { ...state.filters, nodeCategories: toggleIn(state.filters.nodeCategories, action.category) } };
    case "toggleRelationshipType":
      return { ...state, filters: { ...state.filters, relationshipTypes: toggleIn(state.filters.relationshipTypes, action.relationshipType) } };
    case "toggleEvidenceClass":
      return { ...state, filters: { ...state.filters, evidenceClasses: toggleIn(state.filters.evidenceClasses, action.evidenceClass) } };
    case "resetFilters":
      return { ...state, filters: { ...defaultFilterState(), search: state.filters.search }, playing: false };
    case "selectNode": {
      if (!action.id) return { ...state, selectedNode: null, panel: state.panel === "node" ? null : state.panel };
      const fly = action.fly ? { id: action.id, token: (state.cameraTarget?.token ?? 0) + 1 } : state.cameraTarget;
      ...
      if (state.multiSelect) {
        const selection = state.selection.includes(action.id) ? state.selection : [...state.selection, action.id];
        return { ...state, selection, selectedNode: action.id, selectedEdge: null, panel: "selection", cameraTarget: fly };
      }
      return { ...state, selectedNode: action.id, selectedEdge: null, panel: "node", cameraTarget: fly };
    }
    case "selectEdge":
      if (!action.id) return { ...state, selectedEdge: null, panel: state.panel === "edge" ? null : state.panel };
    ...
    case "setMultiSelect":
      return {
        ...
        selection: action.on && state.selectedNode && !state.selection.includes(state.selectedNode) ? [...state.selection, state.selectedNode] : state.selection,
      };
    case "addToSelection":
      if (state.selection.includes(action.id)) return { ...state, multiSelect: true, panel: "selection" };
      return { ...state, multiSelect: true, selection: [...state.selection, action.id], panel: "selection" };
    ...
    case "panel":
      return { ...state, panel: action.panel, multiSelect: action.panel === "selection" ? true : action.panel === null ? false : state.multiSelect };
    case "playing":
      return { ...state, playing: action.on };
    case "escape":
      if (state.playing) return { ...state, playing: false };
      if (state.selectedNode || state.selectedEdge) return { ...state, selectedNode: null, selectedEdge: null, panel: state.panel === "node" || state.panel === "edge" ? null : state.panel };
...
export function useGraphState() {
  ...
  const selectNode = useCallback((id: string | null, fly = false) => dispatch({ type: "selectNode", id, fly }), []);
