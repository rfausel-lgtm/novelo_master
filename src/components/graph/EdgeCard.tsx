...
import { EVIDENCE_CLASS_LABEL, FACT_STATUS_LABEL, RELATIONSHIP_FAMILY_LABEL, RELATIONSHIP_TYPE_LABEL } from "@/lib/schema";
...
interface EdgeCardProps {
  ...
  sourceIndex?: Record<string, SourceInfo>;
  /** Posições dos envolvidos, quando o corpus as trouxer. */
...
export function EdgeCard({ index, edge, onClose, onSelectNode, sourceIndex, positions }: EdgeCardProps) {
  ...
  const typeLabel =
    (RELATIONSHIP_TYPE_LABEL as Record<string, string>)[edge.relationship_type] ?? EDGE_KIND_LABEL[edge.relationship_type] ?? edge.relationship_type;
  ...
  return (
    <PanelShell
      ...
      title={<span className="text-fg-3 text-[10.5px] font-semibold tracking-[0.14em] uppercase">Conexão</span>}
    >
      ...
      {via && (
        <p className="text-fg-3 mt-1.5 text-xs">
          via{" "}
          <button type="button" className="text-fg-2 hover:underline" onClick={() => onSelectNode(via.id)}>
            {via.label}
          </button>
        </p>
      )}
          ...
          {edge.source_ids.map((id) => {
            ...
            return (
              <li key={id} className="text-xs">
                ...
                {(info?.official ?? id.startsWith("fonte-oficial")) && (
                  <span className="border-ev-d text-ev-d ml-1.5 rounded border px-1 text-[10px]">oficial</span>
                )}
              </li>
            );
          })}
      ...
      {edge.evidence_ids.length > 0 && (
        <p className="text-fg-3 mt-1.5 text-[11px]">
          {edge.evidence_ids.length} {edge.evidence_ids.length === 1 ? "evidência vinculada" : "evidências vinculadas"}
        </p>
      )}
      ...
      <p className="text-fg-2 mt-1.5 text-[13px] leading-relaxed">{EVIDENCE_EXPLANATION[edge.evidence_class]}</p>
      <p className="text-fg-3 mt-1 text-[11px]">
        No grafo: {EVIDENCE_SHAPE_LABEL[edge.evidence_class]} ({EVIDENCE_CLASS_LABEL[edge.evidence_class].toLowerCase()}).
      ...
      {positions && positions.length > 0 ? (
        <ul className="space-y-1.5">
          {positions.map((p, i) => (
            <li key={i} className="text-fg-2 text-[13px] leading-relaxed">
              {p.by && <span className="text-fg font-medium">{p.by}: </span>}
              {p.summary}
              {p.date && <span className="text-fg-3"> ({formatDatePT(p.date)})</span>}
            </li>
          ))}
        </ul>
      ) : (
