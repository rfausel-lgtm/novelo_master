...
| `scripts/`                     | Pipeline de dados (`build-data.ts`, `validate-data.ts`), dataset sintético (`synth-stress.ts`), scanner de segredos de fallback, auditoria do acervo (`auditoria.ts`, `auditoria-selecao.ts` — ver [docs/AUDITORIA_NOTURNA.md](docs/AUDITORIA_NOTURNA.md)).                                                                    |
...
| `scripts/lib/auditoria/`       | Conferências da auditoria, cada uma isolada e testável: `integridade.ts` (órfãos), `datas.ts`, `duplicidade.ts`, `link-rot.ts`, `lgpd.ts` (dado pessoal, sempre mascarado), `selecao.ts` (o que a revisão por IA olha na noite), `corpus.ts` e `tipos.ts`.                                                                     |
