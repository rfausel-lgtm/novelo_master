# Estado da pesquisa — revisão estrutural do grafo (2026-09-15)

Revisão feita pelo investigador-chefe depois do ciclo dos lotes 274 a 317, a pedido do editor, enquanto o
Plenário do STF julgava a Pet 16.662. Complementa `ESTADO_DA_PESQUISA_2026-09-05.md`, que continua valendo
para retratações, becos sem saída e canais de acesso. Hipóteses que envolvem pessoas identificáveis e ainda
não têm fonte ficam fora deste arquivo, que é público.

## 1. Medição do grafo (antes do lote 318)

- 276 pessoas, 191 organizações, 244 eventos, 542 relações, 1.320 evidências, 1.243 fontes, 853 perguntas
  abertas.
- Relações por classe: A 212, D 177, C 150, I 3. Daniel Vorcaro tem 75 relações, 55 delas classe A.
  **Correção de diagnóstico (auditoria feita no mesmo dia):** isso não é, em regra, fragilidade. Das 57
  relações A que o envolvem, 19 já têm evidência D ligada, mas 17 dessas são alegações investigativas — o
  documento registra o que a PF afirma, não prova o fato, e o lint trata D em alegação como erro de
  classificação. Promover seria o erro. A fila real de auditoria são as relações A **sem nenhuma evidência**
  ligada (6) e as sustentadas só por evidência A (27), para conferir se a fonte existe e diz o que o rótulo
  afirma.
- 45 pessoas e organizações sem nenhuma relação. **Correção de medição:** 36 delas aparecem no grafo por
  participação em eventos (o build desenha essas arestas); só 9 estavam de fato soltas.
- 1.203 pares em que um verbete cita outra entidade pelo nome sem que haja relação entre as duas — fila
  natural de consolidação, a ser trabalhada por lote, nunca em massa (a menção no texto não diz o tipo
  nem a classe da ligação).
- Script de medição reproduzível: ler todos os YAML de `data/`, contar grau por relação e por participação
  em evento separadamente.

## 2. Linhas de investigação, por prioridade

1. **Consolidação do que já foi lido.** Iniciada no lote 318 (irmãos Conte; Ricardo Siqueira; ITER; Nabas;
   holdings controladoras). Próximo passo: auditar as relações A de Vorcaro sem evidência ligada, conferindo
   fonte e rótulo; alegações investigativas ficam em A ou C mesmo com documento, porque o documento registra a
   alegação, não o fato.
2. **Setor de cemitérios ("mercado da morte").** Cadeia hoje documentada: fundo CARE11 / Brazilian Graveyard
   (administrado pela Master CCTVM até 04/02/2026, depois Mérito DTVM; gerido pela Zion; Master cotista segundo a
   imprensa) → 20% da Cortel → Fabiano Zettel
   conselheiro com e-mail do Master → cautelar de preços funerários → voto de Mendonça. Gatilhos com prazo
   contado de 15/09/2026: Prefeitura de SP e SP Regula (10 dias), CVM (15 dias), cópia do Inquérito Civil
   0695.0000483/2025 do MP-SP. Faltam: histórico da VHR Empreendimentos (cemitério de Sabará) e da
   condenação dos irmãos Schumann, a partir das fontes originais (UOL 18/02/2026, O Bastidor 26/11/2025).
3. **Rede religiosa como canal de acesso.** Três pontas sem costura documental: Assembleia de Deus
   Madureira (Cezinha de Madureira e Mendonça desde 2021), Igreja Batista da Lagoinha (pedido do encontro
   de 14/03/2025, segundo o próprio ministro) e Fabiano Zettel (pastor da Lagoinha, remessas à igreja no
   RIF 140515). Pergunta central: quem são os membros da igreja que pediram o encontro. Regra: pertencer à
   mesma igreja não é vínculo com o esquema. **Resultado negativo já apurado:** Assembleia de Deus Madureira
   e Lagoinha são denominações distintas, sem vínculo institucional localizado — Cezinha atua como operador
   político transversal da bancada evangélica, não como ponte entre as duas igrejas; não forçar essa
   costura. Linha em verificação: papel da liderança nacional da Lagoinha nos repasses de Zettel.
4. **Operação Transparência.** Cadeia: depoimento de Glauber Braga (fev/2025) → Mariângela Fialek
   (dez/2025) → bloqueio de R$ 119,2 mi de Valdemar Costa Neto (jul/2026) → Cezinha de Madureira
   (set/2026). Tudo está em classe C por imprensa. A decisão que autorizou a 3ª fase foi localizada: Pet
   13.428/DF, relator Flávio Dino, assinada em 05/09/2026 e cumprida em 14/09 — em conferência para
   publicação (PDF com fonte corrompida, lido por OCR). A própria decisão trata como desconhecida a identidade
   de um dos interlocutores citados nas mensagens; não presumir.
   Pendentes: os servidores Garigham Amarante Pinto e Nara Brum; os outros casos citados pela PF; o destino
   do pedido da Mesa Diretora da Câmara para anular as provas contra Fialek.
5. **Rioprevidência e o "dono".** Ricardo Siqueira prometeu nomear à PF o político a quem se referia;
   Tharcio Nascimento Salgado segue sem relação por decisão editorial até a leitura da decisão sigilosa da
   Pet 15.686; números dos autos no STF e da ação da PGE-RJ não localizados.
6. **Sefer e Benjamim Botelho de Almeida.** O autor da arguição de suspeição contra Mendonça é o controlador
   da Sefer (mesma entidade no acervo). Processos parados que podem gerar fato datado: PAS CVM
   19957.008699/2019-01 (sem movimento público desde 05/2022) e Apelação Criminal 5003557-34.2021.4.03.6181
   no TRF3 (vista desde 11/06/2026). Verbete de Ascendino Madureira Garcia sem revisão desde 03/09.
7. **Fios herdados do estado de 05/09 ainda abertos:** cautelar do Rio 3049678-51.2025.8.19.0001; quadro
   societário da PKL One na Junta Comercial de SP; relatório do COAF sobre o Metrópoles; relatório da PF de
   30/07/2026 sobre Jaques Wagner (33 MB, não lido).

**Transversal:** Flávio Dino é relator de três frentes ativas (ADPF 1.196, Operação Transparência, Pet
16.669) e tem grau 4 no grafo — suas decisões estão em evidências, não em relações. Aplicar a mesma régua de
neutralidade usada com Mendonça e Moraes.

## 3. Calendário a acompanhar

- Resultado da sessão extraordinária do Plenário sobre a Pet 16.662 (15/09/2026) — o editor comunica.
- ADPF 1.196: respostas da Prefeitura/SP Regula (10 dias) e da CVM (15 dias).
- Relatório final da PF sobre Fabiano Zettel, previsto para o fim de setembro de 2026.
- Vista de Gilmar Mendes no referendo do afastamento de Andrei Rodrigues (prazo regimental de 90 dias
  desde 08/09/2026).

## 4. Regras reforçadas neste ciclo

- Identidade entre homônimos só por documento; coincidência de nome e valor vira evidência classe I com
  `inference_basis`, nunca fusão de entidades (caso Conte, lote 318).
- Não criar relação entre suspeito de vazamento e jornalista citado como destinatário: o texto do verbete
  registra o fato atribuído; a aresta amplificaria relação de fonte.
- Afirmação de pesquisa delegada é verificada por esta sessão antes de publicar; o que não encontra fonte
  própria vira pergunta aberta (lote 316: suposta articulação no Congresso em 2021, com erro factual sobre o
  cargo de Rodrigo Pacheco).
